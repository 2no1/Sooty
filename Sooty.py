...
import base64, hashlib, html.parser, re, json, time, os, socket, strictyaml, urllib.parse, requests, tkinter, tkinter.filedialog
...
def b64Decoder():
    url = str(input(' Enter string: ').strip())
...
def dnsHistory():
    d = str(input(" Enter IP to check: ").strip())
    url = 'https://www.virustotal.com/api/v3/ip_addresses/{}/resolutions'.format(d)
    vt_dns_headers = {'x-apikey':configvars.data['VT_API_KEY']}
    response = requests.get(url, headers=vt_dns_headers)
    print('\nTotal passive DNS results: {}\n'.format(response.json()['meta']['count']))
